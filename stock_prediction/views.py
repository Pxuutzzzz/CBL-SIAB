    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return render(request, 'stock_prediction/predict_result.html', {
                    'result': result,
                    'data': data,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(request, f'Berhasil memprediksi {saved_count} data dari CSV!')
                
                # Convert to JSON for display
                results = df_result.to_dict('records')
                
                return render(request, 'stock_prediction/predict_csv_result.html', {
